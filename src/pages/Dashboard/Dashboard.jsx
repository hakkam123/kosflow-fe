    ...
    <div className="flex items-center justify-between py-4 border-b border-gray-100 last:border-b-0">
      <div className="flex-1">
        <p className="font-medium text-gray-900">
          {payment.penghuni?.nama_lengkap || "Penghuni"}
        </p>
        <p className="text-sm text-gray-500">
          Kamar {payment.penghuni?.kamar_id || "-"}
        </p>
      </div>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    setFlashAlertState(location.state?.flashAlert || consumeFlashAlert());
  }, [location.state]);
  ...
  const stats = useMemo(() => {
    ...
    const totalPendingAmount = pendingBillings.reduce(
      (sum, b) => sum + (b.total_tagihan || b.jumlah_tagihan),
      0,
    );
    const totalOverdueAmount = overdueBillings.reduce(
      (sum, b) => sum + (b.total_tagihan || b.jumlah_tagihan),
      0,
    );
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          <p className="text-3xl font-bold text-blue-600">
            ...
            <span className="text-gray-400 text-lg">
              dari {rooms.length} kamar terisi
            </span>
          </p>
        </div>
      </div>
