...
const DashboardUser = () => {
  ...
  const tenantBillings = useMemo(() => {
    ...
    return billings.filter(b => b.penghuni_id === user?.id || b.penghuni?.email === user?.email);
  }, [billings, user]);

  const latestBill = useMemo(() => {
    return tenantBillings.find(b => b.status_tagihan === 'Belum Bayar' || b.status_pembayaran === 'Belum Lunas' || b.status_pembayaran === 'Overdue') 
      || tenantBillings[0];
  }, [tenantBillings]);

  const paymentHistory = useMemo(() => {
    return tenantBillings.filter(b => b.status_tagihan === 'Lunas' || b.status_pembayaran === 'Terbayar');
  }, [tenantBillings]);

  const handlePayment = () => {
    if (latestBill) {
      alert(`Memproses pembayaran untuk ${latestBill.bulan} ${latestBill.tahun} sebesar ${formatRupiah(latestBill.total_tagihan || latestBill.jumlah_tagihan)}`);
    }
  };
  ...
  if (!user) return <div className="flex items-center justify-center min-h-screen">Loading user profile...</div>;
        ...
        <div className="bg-primary-600 p-6 text-white pt-10 pb-12 relative overflow-hidden">
          ...
          <div className="flex justify-between items-start relative z-10">
            <div>
              ...
              <h1 className="text-2xl font-bold">{user.nama_lengkap || user.username}</h1>
            ...
            <>
              ...
              <div className="space-y-4 mb-8">
                <div className="flex justify-between items-center px-1">
                  <span className="text-xs text-primary-600 font-bold uppercase tracking-widest">Tagihan Aktif</span>
                </div>
                ...
                {latestBill ? (
                  <Card className="p-0 border-none overflow-hidden bg-gradient-to-br from-white to-slate-50 shadow-xl border border-slate-100">
                    <div className="p-5 flex flex-col gap-4">
                      <div className="flex justify-between items-start">
                        ...
                        <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-tight flex items-center gap-1 ${
                          latestBill.status_pembayaran === 'Overdue' || latestBill.status_tagihan === 'Terlambat'
                            ? 'bg-red-100 text-red-700'
                            : 'bg-amber-100 text-amber-700'
                        }`}>
                          <Clock className="w-3 h-3" />
                          {latestBill.status_tagihan || latestBill.status_pembayaran}
                        </div>
                      </div>

                      <div>
                        <p className="text-xs text-slate-500 font-medium">Tagihan {latestBill.bulan} {latestBill.tahun}</p>
                        <h3 className="text-2xl font-black text-slate-900 mt-0.5">{formatRupiah(latestBill.total_tagihan || latestBill.jumlah_tagihan)}</h3>
                        <div className="flex items-center gap-2 mt-2 text-xs text-slate-500">
                          <Calendar className="w-3.5 h-3.5" />
                          Jatuh tempo: <span className="font-bold text-slate-700">{formatDate(latestBill.tanggal_jatuh_tempo)}</span>
                        </div>
                      </div>

                      {(latestBill.status_tagihan === 'Belum Bayar' || latestBill.status_pembayaran === 'Belum Lunas' || latestBill.status_pembayaran === 'Overdue') && (
                        <Button
                  ...
                  <div className="bg-white p-8 rounded-3xl border border-dashed border-slate-200 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-sm font-medium text-slate-400">Tidak ada tagihan aktif</p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm">
                  ...
                  <p className="text-sm font-bold text-slate-800">{user.created_at ? formatDate(user.created_at) : '-'}</p>
                </div>
                <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm">
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Status</p>
                  <p className="text-sm font-bold text-success capitalize">{user.role || 'Penghuni'}</p>
                </div>
              </div>
                    ...
                    paymentHistory.slice(0, 2).map((item) => (
                      <div key={item.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h4 className="text-sm font-bold text-slate-800">{item.bulan} {item.tahun}</h4>
                            <p className="text-[11px] text-slate-400 font-medium">{item.tanggal_bayar ? formatDate(item.tanggal_bayar) : 'Lunas'}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-bold text-slate-800">{formatRupiah(item.total_tagihan || item.jumlah_tagihan)}</p>
                          <p className="text-[10px] text-success font-bold uppercase tracking-tighter">Lunas</p>
                        </div>
                      </div>
                    ))
                  ) : (
                    <p className="text-xs text-center text-slate-400 py-4">Belum ada riwayat pembayaran</p>
                  )}
                </div>
              </div>
            </>
            ...
            <div className="pt-2 space-y-4">
              ...
              <div className="space-y-3">
                {paymentHistory.length > 0 ? (
                  ...
                  <p className="text-sm text-center text-slate-400 py-10">Belum ada riwayat pembayaran</p>
                )}
              </div>
            </div>
          )}
